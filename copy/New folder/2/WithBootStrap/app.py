...
def log():
	...
	if request.method =='POST':
		...
		username1 = json_dict['username']
		password1 = json_dict['password']
		c = g.db.execute("SELECT username, password from user where username = (?)", [username1])
		#g.db.commit()
		user = c.fetchone()
...
# @user.route('/update/', defaults={'username': None})
# @app.route('/update/<username>',methods=['GET','POST'])
# def update(username):
# 	if request.method =='POST':
# 		username1 =  request.form['username']
# 		password = request.form['password']
# 		g.db.execute("update user(username,password) set username=? and password= ? where username=? " , [username1,password,username])
# 		g.db.commit()
# 		curr=g.db.execute('select username,password from user')
# 		users = [dict(username=row[0], password=row[1]) for row in curr.fetchall()]
# 		return render_template('hello.html',users=users)
